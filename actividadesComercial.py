...
file_path_csv = r"C:\Users\Admin Local\Downloads\Informe de Actividades.csv"
file_path_dias_libres = r"C:\Users\Admin Local\OneDrive - PUNTO EMPLEO S.A\Automatizacion\Proyecto reporte de actividades COMERCIAL\Días Libres COMERCIAL\Días Libres COMERCIAL.xlsx"
...
# Manejar el caso en que la ruta del archivo CSV no exista
try:
    data = pd.read_csv(file_path_csv)
except FileNotFoundError:
    print(f"Error: El archivo {file_path_csv} no se encontró. Verifica la ruta y vuelve a intentarlo.")
    exit(1)

# Manejar el caso en que la ruta del archivo Excel no exista
try:
    dias_libres_data = pd.read_excel(file_path_dias_libres)
except FileNotFoundError:
    print(f"Error: El archivo {file_path_dias_libres} no se encontró. Verifica la ruta y vuelve a intentarlo.")
    exit(1)
...
# Cargar archivo CSV con las actividades
data = pd.read_csv(file_path_csv)
...
def obtener_lunes_pasado(fecha_actual):
    ...
    lunes_reciente = fecha_actual - timedelta(days=dia_semana + 7)
    lunes_reciente = lunes_reciente.replace(hour=7, minute=0, second=0, microsecond=0)
    return lunes_reciente

# Calcular el lunes pasado
lunes_pasado = obtener_lunes_pasado(fecha_actual)

# Calcular días festivos dentro del rango (desde hace 7 días hasta hoy)
dias_festivos_en_semana = [pd.to_datetime(day) for day in dias_festivos if lunes_pasado <= pd.to_datetime(day) <= fecha_actual]
festivos = len(dias_festivos_en_semana)

# Función para calcular el objetivo semanal ajustado por festivos y días libres
def calcular_objetivo_semanal_con_dias_libres(persona, festivos, dias_libres_contados):
    if persona in ["Randy Vizcaino", "Kattya Cordero"]:
        objetivo = 12
    else:
        objetivo = 10
    objetivo_ajustado = objetivo - (2 * festivos)
    dias_libres = dias_libres_contados.get(persona, 0)
    objetivo_ajustado -= (2 * dias_libres)
    return max(objetivo_ajustado, 0)

# Cargar archivo con días libres
dias_libres_data = pd.read_excel(file_path_dias_libres)
dias_libres_data['Fecha'] = pd.to_datetime(dias_libres_data['Fecha'])

# Filtrar los días libres que están dentro del rango de la semana actual
dias_libres_en_semana = dias_libres_data[
    (dias_libres_data['Fecha'] >= lunes_pasado) & 
    (dias_libres_data['Fecha'] <= fecha_actual)
]

# Contar los días libres por persona
dias_libres_contados = dias_libres_en_semana.drop('Fecha', axis=1).apply(lambda x: x.count(), axis=0)
...
filtered_data = data[
    (data['Calendarios Start Date and Time'] >= lunes_pasado) &
    (data['Calendarios Estado actividad'] == 'Realizada') &
    (data['Calendarios Asignado a'].isin(personas))
]

# Imprimir el número total de registros filtrados
...
# Recalcular los objetivos por persona considerando días libres y festivos
objetivos_actualizados = {
    persona: calcular_objetivo_semanal_con_dias_libres(persona, festivos, dias_libres_contados) for persona in personas
}

# Actualizar la tabla resumen con los nuevos objetivos ajustados
tabla_resumen['Objetivo Semanal'] = tabla_resumen.index.map(lambda x: objetivos_actualizados.get(x, 10))
...
try:
    ...
    if columnas_existentes:
        ...
        ax.set_title(f'Actividades Semanales con {festivos} Día(s) Festivo(s)')
